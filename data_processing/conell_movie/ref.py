...
def genBatch(data):
    ...
    for doc in data:
        ...
        for i in xrange(maxsentencenum - len(doc)):
            doc.append([0])
    tmp = map(lambda doc: numpy.asarray(map(lambda sentence : sentence + [0]*(m - len(sentence)), doc), dtype = numpy.int32).T, data)                          #[-1]是加在最前面
...
class Dataset(object):
    def __init__(self, filename, emb,maxbatch = 32,maxword = 500):
        ...
        self.docs = []          # each column is a sentence
        self.label = []         # labels for documents
        self.length = []        # number of words of each sentence
        self.sentencenum = []   # count of sentences in documents
        self.wordmask = []
        self.sentencemask = []
        self.maxsentencenum = []# max sentence count in this batch
